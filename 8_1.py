...
for line in read_input():
    ...
    occurence += len([set(x) for x in line.split(' | ')[1].split(' ') if
                      (set(x) == set(map[4])) or (set(x) == set(map[1])) or (set(x) == set(map[7])) or (
                                  set(x) == set(map[8]))])
